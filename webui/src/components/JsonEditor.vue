...
async function load() {
  ...
  error.value = "";
  try {
    const data = await readLocalSubscription(props.name);
...
async function save() {
  ...
  error.value = "";
  try {
    await writeLocalSubscription(props.name, content.value);
    original.value = content.value;
    showSnackbar({ message: "本地订阅已保存", withDismissAction: true });
    emit("saved");
  } catch (err) {
    error.value = err.message;
    showError(err);
...
<template>
  <div class="json-editor">
    ...
    <MiuixCard class="section json-editor__card">
      <Codemirror
        v-model="content"
        class="json-editor__code"
        :extensions="extensions"
        :disabled="loading || saving"
        placeholder="加载中…"
      />
      <MiuixText v-if="error" class="error json-editor__error" type="body2">
        {{ error }}
      </MiuixText>
      <div class="page-actions json-editor__actions">
        <MiuixButton
          type="secondary"

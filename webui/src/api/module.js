import { enableEdgeToEdge, exec, getPackagesInfo, listPackages, moduleInfo } from 'kernelsu'
import { MOCK_PACKAGES, mockGateway } from './mock'

export const isBrowserDev = typeof window !== 'undefined' && !window.ksu
const info = isBrowserDev ? { version: 'Browser Dev', moduleDir: '/data/adb/modules/fluxnet' } : (() => { try { return JSON.parse(moduleInfo()) } catch { return {} } })()
...
async function gateway(command, args = []) {
  ...
  if (isBrowserDev) raw = mockGateway(command, args)
    ...
    const output = await exec(`sh ${quote(script)} ${quote(command)} ${args.map(quote).join(' ')}`)
    if (output.errno !== 0) throw new Error(output.stderr || output.stdout || '模块命令执行失败')
    raw = output.stdout
  }
...
export const replaceAppList = (mode, apps) => gateway('app-list-replace', [mode, encode(JSON.stringify(apps))])
export const getSubscriptions = () => gateway('subscription-list')
export const addRemoteSubscription = (name, url) => gateway('subscription-add-remote', [encode(name), encode(url)])
export const updateSubscription = (name) => gateway('subscription-update', [name])
export const switchSubscription = (name) => gateway('subscription-switch', [name])
export const removeSubscription = (name) => gateway('subscription-remove', [name])
export const createLocalSubscription = (name) => gateway('local-create', [name])
...
export async function getInstalledApps() {
  ...
  try {
    ...
    if (details.length) return details.map(({ packageName, appLabel }) => ({ packageName, appLabel: appLabel || packageName }))
  } catch {}
  const output = await exec('pm list packages')
  return output.stdout.split('\n').filter(Boolean).map((line) => line.replace('package:', '')).map((packageName) => ({ packageName, appLabel: packageName }))
}

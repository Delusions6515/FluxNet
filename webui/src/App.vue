...
watch(
  index,
  (_, previous) =>
    scrollPositions.set(previous, scroller.value?.getScrollTop?.() ?? 0),
  { flush: "pre" },
);
function restoreScroll() {
  scroller.value?.setScrollTop?.(scrollPositions.get(index.value) ?? 0);
}
function syncSnackbarInset() {
  document.documentElement.style.setProperty(
    "--m-snackbar-inset-bottom",
    `${bottomBar.value?.offsetHeight ?? 0}px`,
  );
}
...
onMounted(() => {
  requestEdgeToEdge();
  if (bottomBar.value) {
    observer = new ResizeObserver(syncSnackbarInset);

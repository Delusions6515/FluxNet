...
async function load() { try { settings.value = await getSettings(); selected.value = [...(settings.value.app_proxy_mode === 'whitelist' ? settings.value.proxy_apps : settings.value.bypass_apps)]; apps.value = await getInstalledApps() } catch (err) { error.value = err.message } }
async function saveSetting(key, value) { try { settings.value = await setSetting(key, value); if (key === 'app_proxy_mode') selected.value = [...(value === 'whitelist' ? settings.value.proxy_apps : settings.value.bypass_apps)]; showSnackbar({ message: '设置已保存，等待手动应用配置', withDismissAction: true }) } catch (err) { error.value = err.message } }
...
async function saveApps() { saving.value = true; try { settings.value = await replaceAppList(settings.value.app_proxy_mode, selected.value); showSnackbar({ message: '应用名单已保存，等待手动应用配置', withDismissAction: true }) } catch (err) { error.value = err.message } finally { saving.value = false } }

...
async function load() {
  try {
    ...
    selected.value = [
      ...(settings.value.app_proxy_mode === "whitelist"
        ? settings.value.proxy_apps
        : settings.value.bypass_apps),
    ];
...
async function saveApps() {
  saving.value = true;
  try {
    settings.value = await replaceAppList(
      settings.value.app_proxy_mode,
      selected.value,
    );
    pinned.value = [...selected.value];
    showSnackbar({
  ...
  <div class="page">
    ...
    <MiuixCard class="section"
      ...
      <div class="page-actions">
        <MiuixButton :disabled="saving" @click="saveApps">{{
          saving ? "保存中…" : "保存应用名单"
        }}</MiuixButton>
      </div></MiuixCard

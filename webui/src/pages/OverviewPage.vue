...
async function refresh(force = false) { if (loading.value || (busy.value && !force)) return; loading.value = true; error.value = ''; try { data.value = await getOverview() } catch (err) { error.value = err.message; showError(err) } finally { loading.value = false } }
async function run(action) { if (busy.value || loading.value) return; busy.value = action; try { await serviceAction(action); showSnackbar({ message: action === 'restart' ? '配置已应用并重启' : '服务操作完成', withDismissAction: true }); await refresh(true) } catch (err) { error.value = err.message; showError(err) } finally { busy.value = '' } }
...
<template>
  <div class="page">
    ...
    <MiuixCard class="section"><div class="page-actions"><MiuixButton type="primary" :disabled="Boolean(busy) || loading || status?.running" @click="run('start')">{{ busy === 'start' ? '启动中…' : '启动' }}</MiuixButton><MiuixButton type="secondary" :disabled="Boolean(busy) || loading || !status?.running" @click="run('stop')">{{ busy === 'stop' ? '停止中…' : '停止' }}</MiuixButton><MiuixButton type="secondary" :disabled="Boolean(busy) || loading" @click="run('restart')">{{ busy === 'restart' ? '应用中…' : '应用并重启' }}</MiuixButton><MiuixButton type="secondary" :disabled="Boolean(busy) || loading" @click="refresh">{{ loading ? '刷新中…' : '刷新' }}</MiuixButton></div></MiuixCard>
